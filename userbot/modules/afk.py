...
# ========================= CONSTANTS ============================
AFKSTR = [
    f"**Kang Sadboy {ALIVE_NAME} Sedang Meratapi Nasib!**",
    f"**Maaf Kang Sadboy {ALIVE_NAME} Sedang AFK\n Tunggu Sampai Dia Kembali Online!**",
    f"**Kang Sadboy {ALIVE_NAME} Sedang AFK\n Tunggu Lah Gbllk Sampai Dia Online**",
    f"**Asuu Dibilangin {ALIVE_NAME} Sedang Meratapi Nasib!**",
]
...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"**៚AFK! ツ**\n**Sadboy Telah Dighosting**\
        \n☛ **Alasan:** `{string}`")
    else:
        await afk_e.edit("**៚AFK! ツ**\n**Sadboy Telah Dighosting**")
    if user.last_name:
...
async def type_afk_is_not_true(notafk):
    ...
    if ISAFK:
        ISAFK = False
        msg = await notafk.respond("**Pengumuman Kang Sadboy Comeback!**")
        time.sleep(3)
        await msg.delete()
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"**✘ Sadboy {ALIVE_NAME} Sedang AFK** {afk_since} **Yang Lalu.**\
                        \n☛ **Karena:** `{AFKREASON}`")
                else:
                ...
                if USERS[mention.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await mention.reply(f"**✘ Kang Sadboy Masih AFK** {afk_since} **Yang Lalu.**\
                            \n☛ **Karena:** `{AFKREASON}`")
                    else:
                        await mention.reply(str(choice(AFKSTR)))
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"✘ **Kang Sadboy Sedang AFK** {afk_since} **Yang Lalu**.\
                        \n☛ **Karena**: `{AFKREASON}`")
                else:
